...
`random_bytes()` pulls from the OS CSPRNG. `random_int()` wraps that for integers. These replace every home-grown token generation scheme that was quietly doing it wrong.
...
When `assert.active = 0`, the expression is never evaluated. When it's on, a failing assertion throws the provided exception directly. This is finally a tool you can use without apologizing for it.
...
Before this, you either set options globally in `php.ini` or called `ini_set()` before `session_start()`. Neither was great when you needed different session configurations in different parts of an application.
