...
Previously, extracting named keys from an array required either `extract()` (which dumps variables into scope and invites collisions) or a series of individual assignments. This is cleaner.

## :arrows_counterclockwise: The iterable type

If you write a function that accepts either an array or a generator, there was no clean type hint for that in 7.0. You either typed it as `array` and excluded generators, or dropped the hint entirely:
...
`iterable` accepts anything that can be used in a `foreach` — arrays and `Traversable` implementations both. It also works as a return type. Not dramatic, but it closes a real gap.
...
`ArgumentCountError` extends `TypeError`, which extends `Error`. Callsites that previously silently degraded now fail loudly. That's a migration risk if you have code that relied on the permissive behavior, but it's the right default.
