...
In practice, it was `vagrant up`, wait four minutes, watch the provision fail on a package that changed its download URL, fix it, reprovision, wait again. Vagrantfiles accreted configuration over time — workarounds for specific machines, OS version pinning, memory tweaks for the team member whose laptop had 8GB. The files became historical documents nobody wanted to touch.

The VM itself was the other problem. Booting took time. Running took memory and CPU that could have gone to the application. File syncing between host and guest added latency that made PHP applications feel slower than they should. The tooling overhead was significant for what was ultimately just "run a web server."
...
The contrast with our Vagrant setup was immediate. Not faster by a percentage — faster by a different order. And the Compose file was readable: each service, its image, its volumes, its environment variables, its dependencies. Compared to a provision script that SSHed into a VM and ran apt-get, it was legible.
...
The `services` declaration also replaced the VM provisioning problem. If a new developer joins, they don't run a provision script that may or may not work on their OS version — they run `docker compose up` and get the exact same images everyone else runs.

CI/CD became simpler too. The same `docker-compose.yml` that ran locally could run in the pipeline. The environment parity that Vagrant promised but rarely delivered was real with Compose.
...
We had been using v1 for two years after v2 shipped. Our CI scripts, our Makefiles, our documentation all said `docker-compose`. Nothing had broken because Docker maintained the old binary for a long time. But the ecosystem had moved on silently, and we'd missed it.
...
The hyphen between `docker` and `compose` was always cosmetic. What mattered was the shift from provisioned machines to declarative services — and that shift happened the day we ran a project someone else containerized and realized we never wanted to go back.
